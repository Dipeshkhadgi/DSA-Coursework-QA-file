// Answer Summary:
// The provided solution implements an iterative approach using a stack to traverse the binary search tree in an inorder manner. 
//It maintains a list of the 
// x closest values encountered so far while traversing the tree.
//If the list size is less than 
// x, it adds the current node's value directly to the list.
//Otherwise, it compares the difference between the current node's value and the target value with the maximum difference among the existing closest values.
//If the current node's value has a smaller difference, it replaces the value with the maximum difference in the list.
//This process continues until the entire tree is traversed or the list contains 
